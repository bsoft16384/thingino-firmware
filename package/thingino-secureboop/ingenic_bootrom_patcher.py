...
------------------------
The table lives in the boot-block header. Its *position* is set by the boot
source, not the SoC (verified across the t23/t32/t40/t41/a1 ROMs -- one parser,
two call sites each): NOR/SFC boot reads it at header+0x100 (the default), SD/MSC
boot at header+0x40 (pass --sd; the SD SPL is MSPL-format). Relative to the
magic word::
...
#   NOR/SFC boot -> SPL+0x100 (the per-SoC SocTarget default)
#   SD/MSC boot  -> SPL+0x40  (--sd; the SD SPL is MSPL-format, RSA key at 0x80)
...
def report(
    *, input_path: Path, output_path: Path, soc: str, target: SocTarget,
    addr: int, value: int, boot_offset: int, status: str,
    ...
    medium = "SD/MSC" if target.magic_offset == SD_MAGIC_OFFSET else "NOR/SFC"
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.sd:   # SD/MSC boot: same secure address, different table offset
            target = replace(target, magic_offset=SD_MAGIC_OFFSET, table_limit=SD_TABLE_LIMIT)
        ...
        report(
            ...
            status=status, entries=entries, data=data, wrote=wrote,
        )
        return 0
