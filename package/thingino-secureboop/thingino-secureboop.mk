...
define THINGINO_SECUREBOOP_INIT_TABLE_BYPASS
	@_bin=$$(ls $(@D)/u-boot-with-tpl-mmc-lzma.bin $(@D)/u-boot-with-tpl-lzma.bin $(@D)/u-boot-with-spl-lzma.bin $(@D)/u-boot-lzo-with-spl.bin 2>/dev/null | head -1); \
	if [ -z "$$_bin" ]; then \
		echo "SECUREBOOP: no U-Boot binary found in $(@D)"; exit 1; \
	fi; \
	echo "SECUREBOOP: injecting $(SOC_FAMILY) init-table patch ($$(basename $$_bin))"; \
...
SECUREBOOP_T31_LOAD_HEADER = $(call qstrip,$(BR2_PACKAGE_THINGINO_SECUREBOOP_T31_LOAD_HEADER))
...
define THINGINO_SECUREBOOP_T31_PATCH
	...
	if [ -z "$(SECUREBOOP_T31_MODULUS)" ]; then \
		...
		python3 $(SECUREBOOP_DIR)/ingenic_t31_spl_patcher.py patch \
			...
			$(if $(SECUREBOOP_T31_LOAD_HEADER),--load-header "$(SECUREBOOP_T31_LOAD_HEADER)") \
